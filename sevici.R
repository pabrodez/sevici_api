...
if (!dir.exists("./requests")) {dir.create("./requests")} # create folder where to store requests
...
day_hour <- format.POSIXct(Sys.time(), "%Y-%m-%e %H-%M")  # Create date-time to name each request

write(req_content, file = paste0("./requests/", day_hour, ".json"))  # Save request content

req_list <- list.files("./requests", full.names = TRUE)
json_data <- lapply(req_list, function(x) fromJSON(txt = x, flatten = TRUE))  # convert from json, and get back unnested data frames
...
parent_df$timestamp <- lubridate::ymd_hms(parent_df$timestamp, tz = "Europe/Madrid")
parent_df <- parent_df %>% arrange(desc(timestamp))
...
ggplot() +
  geom_point(data = parent_df[1:258, ], 
